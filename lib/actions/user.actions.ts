...
export async function updateUser({
    ...
    try {
        await User.findOneAndUpdate(
            ...
            { upsert: true }
            // upsert upsert is a database operation that will update an existing row if a specified value already exists in a table, and insert a new row if the specified value doesn't already exist.
        )
...
export async function fetchUserPosts(userId: string){
    try {
        ...
        throw new Error(`No Threads not found ${error.message}`);
